...
def index(request, token):
    try:
        ...
        close_list = close_quiestion.values()
        for question,i in zip(close_quiestion, close_list):
            radio = question.radiotext_set.all()
            i['answer'] = list(radio.values('radioText'))

        

        one_list = list(close_list) + list(open_quiestion.values())
